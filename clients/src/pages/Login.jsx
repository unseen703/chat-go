...
function Login() {
  ...
  const googleSuccess = async (res) => {
    ...
    const result = jwtDecode(decoded);
    const token = res?.credential;

    if (token) {
      console.log(res.profileObj)
      setIsLoading(true)
      const response = await googleAuth({ tokenId: token })
      setIsLoading(false)
      ...
      <div className='bg-[#fff] w-[100vw] h-[100vh] flex justify-center items-center'>
        <div className='w-[90%] sm:w-[400px] pl-0 ml-0 h-[400px] sm:pl-0 sm:ml-9 mt-20 relative'>
          ...
          <div className='absolute -top-5 left-0'>
            ...
            <p className='text-[#000] text-[12px] tracking-wider font-medium'>No Account ? <Link className='text-[#4399ff] underline' to="/register">Sign up</Link></p>
          </div>
          ...
          <form className='flex flex-col gap-y-3 mt-[12%]' onSubmit={formSubmit}>
            ...
            <div className='relative'>
              ...
              {
                !showPass ? <button type='button'><BsEmojiLaughing onClick={() => setShowPass(!showPass)} className='text-[#4399ff] absolute top-3 right-5 sm:right-24 w-[30px] h-[25px]' /></button> : <button type='button'> <BsEmojiExpressionless onClick={() => setShowPass(!showPass)} className='text-[#4399ff] absolute top-3 right-5 sm:right-24 w-[30px] h-[25px]' /></button>
              }
            </div>

            <button style={{ background: "#4399ff" }} className='w-[100%]  sm:w-[80%] h-[50px] font-bold text-[#fff] tracking-wide text-[17px] relative' type='submit'>
